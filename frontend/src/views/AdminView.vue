...
const cacheStats = ref<Record<string, { total: number; last_24h: number }> | null>(null)

const zoneClassOptions = computed(() => {
  const classes = new Set(rules.value.map(r => r.zone_class))
  const sorted = [...classes].sort()
  return [{ value: '', label: 'All' }, ...sorted.map(z => ({ value: z, label: z }))]
})
...
onMounted(async () => {
  try {
    const [s, r, cs] = await Promise.all([api.admin.pipelineStatus(), api.admin.rules(), api.admin.cacheStats()])
    status.value = s
    rules.value = r
    cacheStats.value = cs
  ...
  <div class="h-full overflow-y-auto bg-surface-50">
    <div class="max-w-5xl mx-auto p-8">
      ...
      <div v-else-if="activeTab === 'status' && status" class="space-y-6">
        ...
        <!-- Cache Stats -->
        <div v-if="cacheStats" class="space-y-3">
          <h2 class="text-[10px] font-bold uppercase tracking-widest text-surface-400">Cache Performance</h2>
          <div class="grid grid-cols-3 gap-4">
            <div
              v-for="(stats, key) in cacheStats"
              :key="key"
              class="bg-white rounded-lg border border-surface-200 p-4"
            >
              <p class="text-2xl font-bold text-cover-black tracking-tight">{{ stats.total }}</p>
              <p class="text-[10px] uppercase tracking-widest text-surface-400 mt-1 font-semibold">
                {{ key.replace(/_/g, ' ') }}
              </p>
              <p class="text-[11px] text-surface-300 mt-2">
                <span class="text-cover-green font-medium">+{{ stats.last_24h }}</span> last 24h
              </p>
            </div>
          </div>
        </div>
      ...
      <div v-else-if="activeTab === 'rules'" class="space-y-4">
        <div class="flex items-center gap-4">
          ...
          >
            <option v-for="opt in zoneClassOptions" :key="opt.value" :value="opt.value">{{ opt.label }}</option>
          </select>
        </div>

        <div class="bg-white rounded-lg border border-surface-200 overflow-hidden">
          <div v-if="filteredRules.length === 0" class="text-center py-12 text-surface-400 text-[13px]">No rules found for this filter.</div>
